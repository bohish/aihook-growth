...
/** A readable subject, not the first noisy words of a TikTok caption. */
export function shortTopic(caption: string, maxWords = 6): string | null {
  const text = normalise(caption);
  const tags = [...text.matchAll(/#([^#\n]+)/g)]
    .map((m) => clean(m[1] ?? ""))
    .filter((tag) => tag && !STOP.has(tag.toLowerCase()));
  ...
  const career = [...tags, words.join(" ")].find((part) => /مسيره لاعب|مسيرة لاعب/i.test(part));
  if (career && fc) return `مسيرة لاعب في ${fc.replace(/\s/g, "")}`;
  const tagSubject = tags.find(
    (tag) => tag.split(" ").filter((word) => !STOP.has(word.toLowerCase())).length >= 2,
  );
  const chosen = tagSubject ? tagSubject.split(" ") : words;
  if (chosen.length === 0) return fc ? fc.replace(/\s/g, "") : null;
  return (
    chosen
...
export function buildAccountContext(
  ...
  return {
    ...
    nicheLabel:
      niche === "unknown"
        ? "غير محدد"
        : NICHE_WORDS[detected as Exclude<NicheKey, "unknown">].label,
    topics,
    entities,
    topTopic,
    bestStoredHookType,
