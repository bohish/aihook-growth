...
const inputSchema = z.object({ hook: z.string().min(3).max(2000) });
export type HookAgentResult = | { ok: true; json: string } | { ok: false; error: string; missing?: string[] };
  ...
  .handler(async ({ data }): Promise<HookAgentResult> => {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ source: "lovable-aihook", event: "hook_analysis", payload: { hook: data.hook } }),
      });
