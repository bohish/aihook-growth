...
export function buildWeeklyPlanFocused(
  ...
  const durGroups = (() => {
    const map = new Map<string, VideoRecord[]>();
    videos.forEach((v) => map.set(v.features.durationBucket, [...(map.get(v.features.durationBucket) ?? []), v]));
    return [...map.entries()]
  ...
  const targetDuration = durGroups[0] ? durLabel(durGroups[0].key) : durLabel(sorted[0]!.features.durationBucket);
